...
public class EmpProc {
	...
	public void execute() {
		...
		while (true) {
			System.out.println("메뉴실행.  1)단건조회  2)전체조회 3)입력 4)salary수정 9)종료");
			menu = sc.nextInt();
			if (menu == 1)
				getEmpProc();
			else if (menu == 2)
...
//	}
	...
	public void insertEmp() {
		//EmpServiceImpl service = new EmpServiceImpl();		//EmpServiceImpl service = EmpServiceImpl.getInstance(); 로 대체
		System.out.println("아이디를 입력:");
		int empId = sc.nextInt();sc.nextLine();		//sc.nextLine(); 안써주면 엔터키가 살아서 다음 입력에 먹힘   sc.nextInt();가 숫자만 먹기때문
	...
	public void getEmpListProc() {
		//EmpService service = new EmpServiceImpl();		//EmpServiceImpl service = EmpServiceImpl.getInstance(); 로 대체
